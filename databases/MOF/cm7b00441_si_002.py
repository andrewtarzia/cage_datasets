...
if os.path.isdir(solvent_file):
    ...
    f = open(html_file, "w")
    f.write('<br>')
    f.write('Sorry, unable to locate solvent files in CSD_2017 directory')
    f.write('<br>')
    f.close()
...
if entry.has_3d_structure:
    ...
    with io.CrystalWriter('%s/%s_stripped.cif' % (helper.options['working_directory_path'], entry.identifier)) as writer:
